...
from utils import show_inpainting_reconstructions, imshow, remove_patch, save_checkpoint, load_checkpoint
import argparse
import random
import numpy as np
import os
...
def parse_args():
    p = argparse.ArgumentParser(description="Train autoencoder for CIFAR-10 dog image inpainting")
...
def main():
    ...
    for epoch in tqdm(range(num_epochs), desc="Training"):
        ...
        for images, _ in train_loader:
            ...
            loss.backward()
            optimizer.step()
            epoch_loss += loss.item() * patch_images.size(0)

        if epoch % epochs_per_summary == 0:
            show_inpainting_reconstructions(model, test_loader, device, n_show=8, fname=f"epoch_{epoch:03d}.png")
            print(f"Epoch {epoch:>4d} | MSE {epoch_loss/len(train_loader.dataset):.4f} | "
                  f"Test MSE {model.validate_inpainting(test_loader, device):.4f}")
